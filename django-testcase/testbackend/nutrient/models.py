...
class Feed(models.Model):
    ...
    calorie = models.CharField("칼로리(kcal/kg)", max_length=1000)
    ...
    # 환산 기준
    standard = models.CharField("기준량", max_length=1000, default="", blank=True, null=True)
...
class ExchangedFeed(models.Model):
    ...
    types = models.CharField("분류", max_length=2, choices=TYPE_CHOICES)  
    # 칼로리 수분량 조단백 조지방 조섬유 조회분 칼슘 인
    calorie = models.CharField("칼로리(kcal/1g)", max_length=1000)
    moisture = models.CharField("수분량(ml/1g)", max_length=1000)
    crude_protein = models.CharField("조단백(g/1g)", max_length=1000)
    crude_fat = models.CharField("조지방(g/1g)", max_length=1000)
    crude_fiber = models.CharField("조섬유(g/1g)", max_length=1000)
    crude_ash = models.CharField("조회분(g/1g)", max_length=1000)
    calcium = models.CharField("칼슘(g/1g)", max_length=1000)
    phosphorus = models.CharField("인(g/1g)", max_length=1000)
